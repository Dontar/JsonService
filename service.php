...
// $servicesPath = dirname(__FILE__) . DIRECTORY_SEPARATOR . "services";
$servicesPath = dirname(__FILE__);
...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
    ...
    foreach ( $input as $calls ) {
        try {
            require_once $servicesPath . DIRECTORY_SEPARATOR . $calls['service'] . ".php";
    ...
    $script = "";
    $dir = scandir($servicesPath);
    foreach ( $dir as $file ) {
        if (($p = strpos($file, ".php")) !== false && $file != "service.php") {
            $service = substr($file, 0, $p);
            try {
                require_once $servicesPath . DIRECTORY_SEPARATOR . $file;
            }
            catch ( Exception $e ) {
                continue;
            }
